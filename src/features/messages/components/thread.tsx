...
export const Thread = ({
    ...
    const { results: messages, loadMore, status } = useGetMessages(
        {
            channelId,
            parentMessageId: messageId
        }
                ...
                {Object.entries(groupedMessages || {}).map(([dateKey, messages]) => (
                    <div key={dateKey}>
                        ...
                        {messages.map((message, index: number) => {
                            // compare creation time between previous message and current message
                            // if difference < 5 minnutes + same user => needn't show avatar image of user
